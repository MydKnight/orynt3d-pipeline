...
**In scope:** all exported pure functions in `src/profiles/` (extraction helpers), `src/pipeline/filter.ts`, `src/pipeline/config-writer.ts`, and the `src/pipeline/session.ts` save/load roundtrip (a plain temp-dir filesystem test, no mocking).

**Exempt:** CLI (`src/cli.ts`), TUI stages (`tagger.ts`, `reviewer.ts`), and I/O stages (`extractor.ts`, `reorganizer.ts`, `classifier.ts`) — these are interactive or bound to the NAS / archive libraries and not unit-testable without heavy mocking that adds no real value.

Current state: 71 tests across 7 files — all in-scope areas covered.
