...
- Category from the ZIP filename (`SUPPORTED NPCs` / `SUPPORTED Monsters` / `Terrain`); tolerates a `(1)` browser suffix. NPCs prompt hero/npc, Monsters auto `monster`, Terrain auto `terrain`.
...
- Five pipeline profiles (Loot Studios, Flesh of Gods, Rescale, Archvillain Games, DM Stash) implemented and verified
...
- Only `docker/` needs to be on the QNAP; the container clones the repo at runtime, so the laptop dev loop is: edit `src/` → `git push origin master` → next `docker compose run` picks it up (`git fetch` + `reset --hard`).
...
Current state: 124 tests across 14 files — all in-scope areas covered.
