...
- **Pack:** wrapper holds multiple `{Name}_Supports/` pose subfolders — one `ClassifiedModel` per pose. A `..._Base_Supports/` folder is shared geometry, not a pose — it doesn't get its own model; its files are merged into every real pose's file list (`isSharedBaseFolder`), except a pose that already ships its own base file (`hasOwnBaseFile`) — some poses in a pack do, some don't, and giving one both is wrong, not redundant-but-harmless.
...
Current state: 134 tests across 15 files — all in-scope areas covered.
