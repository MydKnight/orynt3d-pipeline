...
**v1 + v2 complete and running in production.**

- Both pipeline profiles (Loot Studios, Flesh of Gods) implemented and verified
- Orynt3D migration to subscription-level sources complete for both subscriptions
- Release tracking DB live: `data/pipeline.db` records every import with subscription, pack name, month, and owned/processed state
- `npm run pipeline` prompts to mark the release processed after NAS write; warns if already imported
- `npm run download` provides status, list, backfill (historical imports), and manual ownership tracking
...
- **DM Stash** — not on MyMiniFactory; has its own site. Needs a dedicated platform implementation. Investigate site structure and whether Cloudflare or similar bot protection is in use before building. Profile (`src/profiles/dmstash.ts`) also needed once download folder structure is known.
...
1. **Backfill historical imports** — run `npm run download` → backfill for each subscription to record releases already in Orynt3D before tracking was added
2. **Flesh of Gods profile** — add Rescale as a third pipeline profile once a raw download is available to inspect
...
5. **Additional pipeline profiles** — DM Stash, Archvillain Games, Witchsong Miniatures (each needs a raw download tree first)
...
- **v2: Release tracking** (in progress) — SQLite DB with four ownership states; MMF API discovery for FoG and Rescale; manual download tracking; status report CLI
