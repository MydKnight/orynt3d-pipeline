...
- Release tracking DB built (`data/pipeline.db`) — tracks subscription, pack name, month, and owned/processed state. **Not yet populated:** the DB started empty and predates the move off OneDrive; historical backfill is the #1 open next action. The `.db` file is committed to git (public repo, accepted) so it persists as the backup of record going forward.
...
4. **Additional pipeline profiles** — DM Stash, Archvillain Games, Witchsong Miniatures (each needs a raw download tree first)
